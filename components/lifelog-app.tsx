...
import { AlertCircle, Archive, BarChart3, CalendarDays, Check, ChevronLeft, ChevronRight, CircleUserRound, Cloud, CloudOff, Coffee, Download, History, Home, LoaderCircle, LogOut, Pencil, Plus, RotateCcw, Settings2, ShieldCheck, SlidersHorizontal, Sparkles, Target, Trash2, X } from "lucide-react";
...
export function QuickLog({ metric, entries, onSave }: { metric: Metric; entries: Entry[]; onSave: (metric: Metric, value: EntryValue, date?: string, note?: string) => void }) { const [amount, setAmount] = useState("1"); const [note, setNote] = useState(""); const [logged, setLogged] = useState(false);
  useEffect(() => { if (!logged) return; const timer = window.setTimeout(() => setLogged(false), 1800); return () => window.clearTimeout(timer); }, [logged, entries.length]); const total = entries.reduce((sum, entry) => sum + Number(entry.value), 0); const save = () => { if (!amount.trim() || !Number.isFinite(Number(amount))) return; onSave(metric, Number(amount), undefined, note); setNote(""); setLogged(true); }; return <article className={`quick-card ${metric.notesEnabled ? "with-note" : ""}`} style={{ "--metric": COLORS[metric.color] } as React.CSSProperties}><span className="quick-icon"><MetricGlyph metric={metric} size={21} /></span><div><strong>{metric.name}</strong><span role="status">{logged ? "Entry recorded" : `${total} ${metric.unit ?? ""} today`}</span></div><input aria-label={`${metric.name} amount`} type="number" inputMode="decimal" step="any" value={amount} onChange={(event) => setAmount(event.target.value)} /><button aria-label={`Log ${metric.name}`} disabled={!amount.trim() || !Number.isFinite(Number(amount))} onClick={save}>{logged ? <Check size={20} /> : <Plus size={20} />}</button>{metric.notesEnabled && <input className="quick-note" aria-label={`${metric.name} notes`} maxLength={500} value={note} onChange={(event) => setNote(event.target.value)} placeholder="Optional note…" />}</article>; }
