...
import { AlertCircle, BarChart3, CalendarDays, Check, ChevronLeft, ChevronRight, CircleUserRound, Cloud, CloudOff, Coffee, Download, History, Home, LoaderCircle, LogOut, Minus, Pencil, Plus, RotateCcw, Settings2, ShieldCheck, SlidersHorizontal, Sparkles, Target, Trash2, X } from "lucide-react";
...
import type { Entry, EntryValue, FrequencyType, LifeLogData, LoggingMode, Metric, MetricColor, MetricType, ScheduleType } from "@/lib/types";
...
function fromCloudMetric(row: Record<string, unknown>): Metric { return { id: String(row.id), name: String(row.name), description: row.description ? String(row.description) : undefined, type: row.type as MetricType, unit: row.unit ? String(row.unit) : undefined, loggingMode: row.logging_mode as LoggingMode, schedule: row.schedule as ScheduleType, weekdays: row.weekdays as number[] | undefined, color: row.color as MetricColor, ratingMin: row.rating_min as number | undefined, ratingMax: row.rating_max as number | undefined, options: row.options as string[] | undefined, aggregation: row.aggregation as Metric["aggregation"], notesEnabled: Boolean(row.notes_enabled), frequency: (row.frequency as FrequencyType) ?? "once", timesPerDay: Number(row.times_per_day ?? 1), intervalHours: Number(row.interval_hours ?? 1), scheduleTimes: row.schedule_times as string[] | undefined, notificationsEnabled: Boolean(row.notifications_enabled), archived: Boolean(row.archived_at) }; }
function fromCloudEntry(row: Record<string, unknown>): Entry { return { id: String(row.id), metricId: String(row.metric_id), value: row.value as EntryValue, occurredAt: String(row.occurred_at), localDate: String(row.local_date), timezone: String(row.timezone), slotKey: String(row.slot_key ?? "daily"), note: row.note ? String(row.note) : undefined, synced: true }; }
function cloudMetric(metric: Metric, userId: string | null) { return { id: metric.id, user_id: userId, name: metric.name, description: metric.description, type: metric.type, unit: metric.unit, logging_mode: metric.loggingMode, schedule: metric.schedule, weekdays: metric.weekdays, color: metric.color, rating_min: metric.ratingMin, rating_max: metric.ratingMax, options: metric.options, aggregation: metric.aggregation, notes_enabled: metric.notesEnabled ?? false, frequency: metric.frequency ?? "once", times_per_day: metric.timesPerDay ?? 1, interval_hours: metric.intervalHours ?? 1, schedule_times: metric.scheduleTimes ?? [], notifications_enabled: metric.notificationsEnabled ?? false, archived_at: metric.archived ? new Date().toISOString() : null }; }
...
function useLifeLog(userId: string | null) {
  ...
  const saveEntry = (metric: Metric, value: EntryValue, date = day(), note?: string, slotKey = "daily") => setData((current) => {
    if (!validEntryValue(metric, value)) return current;
    const timezone = Intl.DateTimeFormat().resolvedOptions().timeZone;
    const cleanNote = metric.notesEnabled ? note?.trim() || undefined : undefined;
    const resolvedSlot = metric.loggingMode === "event" ? crypto.randomUUID() : slotKey;
    const base: Entry = { id: crypto.randomUUID(), metricId: metric.id, value, occurredAt: new Date().toISOString(), localDate: date, timezone, slotKey: resolvedSlot, note: cleanNote, synced: false };
    const entries = metric.loggingMode === "daily"
      ? [...current.entries.filter((entry) => !(entry.metricId === metric.id && entry.localDate === date && (entry.slotKey ?? "daily") === resolvedSlot)), base]
      : [...current.entries, base];
    send({ id: metric.loggingMode === "daily" ? `entry:${metric.id}:${date}:${resolvedSlot}` : `entry:${base.id}`, kind: "entry", payload: { id: base.id, user_id: userId, metric_id: metric.id, value, occurred_at: base.occurredAt, local_date: date, timezone, slot_key: metric.loggingMode === "daily" ? resolvedSlot : base.id, note: cleanNote } });
    return { ...current, entries };
  });
  const updateEntry = (entry: Entry, value: EntryValue, note?: string, date = entry.localDate) => { const metric = data.metrics.find((item) => item.id === entry.metricId); if (!metric || !validEntryValue(metric, value)) return; const cleanNote = metric.notesEnabled ? note?.trim() || undefined : entry.note; const updated = { ...entry, value, note: cleanNote, localDate: date, synced: false }; setData((current) => ({ ...current, entries: current.entries.map((item) => item.id === entry.id ? updated : item) })); send({ id: `entry:${entry.id}`, kind: "update-entry", payload: { id: entry.id, value, local_date: date, note: cleanNote } }); };
...
function LifeLogWorkspace({ userId }: { userId: string | null }) {
  ...
  useEffect(() => {
    const navigate = () => {
      ...
      if (["today", "history", "metrics", "insights", "settings"].includes(next)) {
        (document.activeElement as HTMLElement | null)?.blur();
        updateTab(next as Tab);
      } else updateTab("today");
    };
    navigate();
...
function Sidebar({ tab, setTab, online, syncStatus, pendingCount, onSync }: { tab: Tab; setTab: (tab: Tab) => void; online: boolean; syncStatus: SyncStatus; pendingCount: number; onSync: () => void }) {
  ...
  return <aside className="sidebar"><Brand /><nav aria-label="Main navigation">{links.map(([id, NavIcon, label]) => <button key={id} aria-current={tab === id ? "page" : undefined} className={tab === id ? "active" : ""} onClick={() => setTab(id)}><NavIcon size={19} />{label}</button>)}</nav><div className="sidebar-foot"><button className={`sync-state ${syncStatus}`} onClick={onSync} disabled={!online || syncStatus === "local"}><Icon size={16} /><span>{label}</span></button><button className="profile" onClick={() => setTab("settings")}><span><Settings2 size={18} /></span><div><strong>Settings</strong><small>Private space</small></div><Settings2 size={17} /></button></div></aside>;
}

function MobileHeader({ online, syncStatus, pendingCount, onSync }: { online: boolean; syncStatus: SyncStatus; pendingCount: number; onSync: () => void }) {
  const label = !online ? "Offline" : syncStatus === "local" ? "On this device" : syncStatus === "syncing" ? "Syncing…" : syncStatus === "error" ? "Retry sync" : pendingCount ? `${pendingCount} pending` : "Synced";
  const Icon = !online ? CloudOff : syncStatus === "error" ? AlertCircle : syncStatus === "syncing" ? LoaderCircle : Cloud;
  return <header className="mobile-head"><Brand /><button className={`mobile-sync ${syncStatus}`} disabled={!online || syncStatus === "local" || syncStatus === "syncing"} onClick={onSync} aria-label={`${label}${syncStatus === "error" ? ". Tap to retry" : ""}`}><Icon size={16} /><span role="status">{label}</span></button></header>;
}
export function BottomNav({ tab, setTab }: { tab: Tab; setTab: (tab: Tab) => void }) { const items: [Tab, typeof Home, string][] = [["today", Home, "Today"], ["history", History, "History"], ["metrics", SlidersHorizontal, "Metrics"], ["insights", BarChart3, "Insights"], ["settings", Settings2, "Settings"]]; return <nav className="bottom-nav" aria-label="Main navigation">{items.map(([id, Icon, label]) => <button key={id} aria-current={tab === id ? "page" : undefined} className={tab === id ? "active" : ""} onClick={() => setTab(id)}><Icon size={21} /><span>{label}</span></button>)}</nav>; }
...
function metricSlots(metric: Metric) {
  if (metric.loggingMode === "event") return [{ key: "event", label: undefined }];
  if (metric.frequency === "interval") {
    const interval = Math.max(1, metric.intervalHours ?? 1); const start = metric.scheduleTimes?.[0]; const startMinutes = start ? Number(start.slice(0, 2)) * 60 + Number(start.slice(3, 5)) : 0;
    return Array.from({ length: Math.ceil((1440 - startMinutes) / (interval * 60)) }, (_, index) => { const minutes = startMinutes + index * interval * 60; const time = `${String(Math.floor(minutes / 60)).padStart(2, "0")}:${String(minutes % 60).padStart(2, "0")}`; return { key: start ? time : `slot-${index + 1}`, label: start ? time : `Check-in ${index + 1}` }; });
  }
  const count = metric.frequency === "times" ? metric.timesPerDay ?? 1 : 1;
...
function validEntryValue(metric: Metric, value: EntryValue) {
  if (metric.type === "number" || metric.type === "rating") return String(value).trim() !== "" && Number.isFinite(Number(value));
  if (metric.type === "boolean") return typeof value === "boolean";
...
export function MetricCard({ metric, slot, entry, onSave, grouped = false, locked = false }: { grouped?: boolean; locked?: boolean; metric: Metric; slot: { key: string; label?: string }; entry?: Entry; onSave: (metric: Metric, value: EntryValue, date?: string, note?: string, slotKey?: string) => void }) {
  ...
  const save = (nextValue = value, nextNote = note) => {
    if (!validEntryValue(metric, nextValue)) return;
    const normalized = metric.type === "number" ? Number(nextValue) : nextValue;
    if (entry?.value === normalized && (entry?.note ?? "") === nextNote.trim()) return;
    onSave(metric, normalized, undefined, nextNote, slot.key);
  };
  ...
  return <Container aria-label={grouped ? slot.label : undefined} className={`${grouped ? "check-in-section" : "metric-card"} ${complete ? "complete" : ""} ${waiting ? "waiting" : ""}`} style={{ "--metric": COLORS[metric.color] } as React.CSSProperties}><header>{!grouped && <span className="metric-icon"><MetricGlyph metric={metric} /></span>}<div>{grouped ? <h4>{slot.label ? displayTime(slot.label) : "Check-in"}</h4> : <><h3>{metric.name}{slot.label && <small className="slot-label">{displayTime(slot.label)}</small>}</h3><p>{metric.description}</p></>}</div>{complete && <span className="check" role="img" aria-label="Recorded"><Check size={15} /></span>}</header>
    {waiting ? <div className="metric-waiting"><p>Available at {displayTime(slot.key)}</p><button className="secondary" onClick={() => setUnlocked(true)}>Set it now</button></div> : <div className="metric-control">
      ...
      {metric.type === "number" && <><div className={`number-stepper ${showNumberError && !valid ? "invalid" : ""}`}><button aria-label={`Decrease ${metric.name} by 1`} onClick={() => select((valid ? Number(value) : 0) - 1)}><Minus size={18} /></button><label className="number-input"><input aria-label={metric.name} aria-invalid={showNumberError && !valid} type="number" inputMode="decimal" required step="any" value={String(value)} onChange={(e) => { setValue(e.target.value); setShowNumberError(false); }} enterKeyHint="done" onKeyDown={(event) => { if (event.key === "Enter") event.currentTarget.blur(); }} onBlur={() => { setShowNumberError(!valid); save(); }} /><span>{metric.unit}</span></label><button aria-label={`Increase ${metric.name} by 1`} onClick={() => select((valid ? Number(value) : 0) + 1)}><Plus size={18} /></button></div>{showNumberError && !valid && <small className="field-error" role="alert">Enter a number.</small>}</>}
...
export function EventMetricCard({ metric, entries, onSave, onUpdate, onDelete }: { metric: Metric; entries: Entry[]; onSave: (metric: Metric, value: EntryValue, date?: string, note?: string) => void; onUpdate: (entry: Entry, value: EntryValue, note?: string) => void; onDelete: (entry: Entry) => void }) {
  const [note, setNote] = useState("");
  const total = entries.reduce((sum, entry) => sum + Number(entry.value), 0);
  const decrease = () => { const latest = [...entries].reverse().find((entry) => Number(entry.value) > 0); if (!latest) return; if (Number(latest.value) > 1) onUpdate(latest, Number(latest.value) - 1, latest.note); else onDelete(latest); };
  const increase = () => { onSave(metric, 1, undefined, note); setNote(""); };
  return <article className="metric-card event-metric" style={{ "--metric": COLORS[metric.color] } as React.CSSProperties}><header><span className="metric-icon"><MetricGlyph metric={metric} /></span><div><h3>{metric.name}</h3><p>{metric.description || "Log it whenever it happens."}</p></div></header><div className="event-total"><strong>{total}</strong><span>{metric.unit} today</span></div><div className="event-stepper"><button aria-label={`Decrease ${metric.name} by 1`} disabled={total <= 0} onClick={decrease}><Minus size={20} /></button><button aria-label={`Increase ${metric.name} by 1`} onClick={increase}><Plus size={20} /></button></div>{metric.notesEnabled && <NoteField value={note} onChange={setNote} />}</article>;
}

function HistoryView({ metrics, entries, onAdd, onUpdate, onDelete }: { metrics: Metric[]; entries: Entry[]; onAdd: (metric: Metric, value: EntryValue, date?: string, note?: string, slotKey?: string) => void; onUpdate: (entry: Entry, value: EntryValue, note?: string) => void; onDelete: (entry: Entry) => void }) {
  ...
  return <div className="page"><PageHeader eyebrow="Your record" title="History" copy="Look back without judgment. Every entry is context." action={<button className="primary" onClick={() => setAdding(true)}><Plus size={16} />Add past entry</button>} /><div className="history-toolbar"><div className="month-nav"><button aria-label="Previous month" onClick={() => moveMonth(-1)}><ChevronLeft size={17} /></button><strong>{monthLabel}</strong><button aria-label="Next month" onClick={() => moveMonth(1)} disabled={month >= day().slice(0, 7)}><ChevronRight size={17} /></button></div><label>Filter<span className="sr-only"> by metric</span><select value={metricFilter} onChange={(event) => setMetricFilter(event.target.value)}><option value="all">All metrics</option>{metrics.map((metric) => <option key={metric.id} value={metric.id}>{metric.name}</option>)}</select></label></div>{dates.length === 0 ? <div className="empty-state"><CalendarDays size={28} /><h2>No entries here yet</h2><p>Choose another month or add an entry for a day you missed.</p><button className="secondary" onClick={() => setAdding(true)}>Add an entry</button></div> : <section className="history-list">{dates.map((date) => <article key={date}><div className="history-date"><CalendarDays size={18} /><div><strong>{prettyDate(date)}</strong><span>{date === day() ? "Today" : new Intl.DateTimeFormat("en-US", { weekday: "long" }).format(new Date(`${date}T12:00:00`))}</span></div></div><div className="history-values">{filtered.filter((entry) => entry.localDate === date).map((entry) => { const metric = metrics.find((item) => item.id === entry.metricId); if (!metric) return null; return <div className="history-entry" key={entry.id}><div><i style={{ background: COLORS[metric.color] }} /><span>{metric.name}</span>{entry.note && <small className="history-note">{entry.note}</small>}<strong>{typeof entry.value === "boolean" ? entry.value ? "Yes" : "No" : entry.value} {metric.unit}</strong></div><div className="history-entry-actions"><button aria-label={`Edit ${metric.name} entry`} onClick={() => setEditing({ entry, metric })}><Pencil size={14} /></button><button aria-label={`Delete ${metric.name} entry`} onClick={() => remove(entry, metric)}><Trash2 size={14} /></button></div></div>; })}</div></article>)}</section>}{editing && <EditEntryModal {...editing} onClose={() => setEditing(null)} onSave={(value, note) => { onUpdate(editing.entry, value, note); setEditing(null); }} />}{adding && <BackfillModal metrics={metrics} onClose={() => setAdding(false)} onSave={(metric, value, date, note, slotKey) => { onAdd(metric, value, date, note, slotKey); setMonth(date.slice(0, 7)); setAdding(false); }} />}</div>;
}

function EditEntryModal({ entry, metric, onClose, onSave }: { entry: Entry; metric: Metric; onClose: () => void; onSave: (value: EntryValue, note?: string) => void }) {
  const [value, setValue] = useState<EntryValue>(entry.value);
  const [note, setNote] = useState(entry.note ?? "");
  const [date, setDate] = useState(entry.localDate);
  // Keep the existing callback API while passing its entry's newly selected date.
  // eslint-disable-next-line react-hooks/immutability
  return <AppDialog onClose={onClose}><form className="modal entry-modal" onSubmit={(event) => { event.preventDefault(); if (validEntryValue(metric, value)) { entry.localDate = date; onSave(metric.type === "number" ? Number(value) : value, note); } }}><header><div><span className="eyebrow">Entry details</span><h2 tabIndex={-1}>Edit {metric.name}</h2></div><button type="button" aria-label="Close" onClick={onClose}><X size={20} /></button></header><label>Date<input required type="date" max={day()} value={date} onChange={(event) => setDate(event.target.value)} /></label><div className="metric-control">{metric.type === "rating" && <div className="rating" style={{ "--range-fill": `${((Number(value) - (metric.ratingMin ?? 1)) / Math.max(1, (metric.ratingMax ?? 10) - (metric.ratingMin ?? 1))) * 100}%` } as React.CSSProperties}><input aria-label={metric.name} type="range" min={metric.ratingMin ?? 1} max={metric.ratingMax ?? 10} value={Number(value)} onChange={(event) => setValue(Number(event.target.value))} /><div><span>{metric.ratingMin ?? 1}</span><strong>{value}</strong><span>{metric.ratingMax ?? 10}</span></div></div>}{metric.type === "boolean" && <div className="boolean"><button type="button" className={value === true ? "selected" : ""} onClick={() => setValue(true)}>Yes</button><button type="button" className={value === false ? "selected" : ""} onClick={() => setValue(false)}>No</button></div>}{metric.type === "number" && <label className="number-input"><input aria-label={metric.name} type="number" inputMode="decimal" required step="any" value={String(value)} onChange={(event) => setValue(event.target.value)} /><span>{metric.unit}</span></label>}{metric.type === "text" && <textarea aria-label={metric.name} value={String(value)} onChange={(event) => setValue(event.target.value)} />}{metric.type === "choice" && <select aria-label={metric.name} value={String(value)} onChange={(event) => setValue(event.target.value)}>{metric.options?.map((option) => <option key={option}>{option}</option>)}</select>}</div>{metric.notesEnabled && <NoteField value={note} onChange={setNote} />}<footer><button type="button" className="secondary" onClick={onClose}>Cancel</button><button className="primary" type="submit">Save changes</button></footer></form></AppDialog>;
}
...
function BackfillModal({ metrics, onClose, onSave }: { metrics: Metric[]; onClose: () => void; onSave: (metric: Metric, value: EntryValue, date: string, note?: string, slotKey?: string) => void }) {
  ...
  const setValue = (key: string, value: EntryValue) => setValues((current) => ({ ...current, [key]: value }));
  const submit = (event: React.FormEvent) => { event.preventDefault(); active.forEach((metric) => metricSlots(metric).forEach((slot) => { const key = `${metric.id}:${slot.key}`; const value = values[key]; if (value !== undefined && validEntryValue(metric, value)) onSave(metric, metric.type === "number" ? Number(value) : value, date, undefined, slot.key); })); };
  return <AppDialog onClose={onClose}><form className="modal backfill-modal" onSubmit={submit}><header><div><span className="eyebrow">Backfill your record</span><h2 tabIndex={-1}>Add past entries</h2><p>Fill in whichever metrics you remember. Leave the rest blank.</p></div><button type="button" aria-label="Close" onClick={onClose}><X size={20} /></button></header><label className="backfill-date">Date<input required type="date" max={day()} value={date} onChange={(event) => setDate(event.target.value)} /></label><section className="backfill-grid">{active.flatMap((metric) => metricSlots(metric).map((slot) => { const key = `${metric.id}:${slot.key}`; const value = values[key]; return <article className="metric-card" key={key} style={{ "--metric": COLORS[metric.color] } as React.CSSProperties}><header><span className="metric-icon"><MetricGlyph metric={metric} /></span><div><h3>{metric.name}{slot.label && <small className="slot-label">{displayTime(slot.label)}</small>}</h3><p>{metric.description}</p></div></header><div className="metric-control">{metric.type === "rating" && <div className="rating"><input aria-label={metric.name} type="range" min={metric.ratingMin ?? 1} max={metric.ratingMax ?? 10} value={Number(value ?? metric.ratingMin ?? 1)} onChange={(event) => setValue(key, Number(event.target.value))} /><div><span>{metric.ratingMin ?? 1}</span><strong>{value === undefined ? "—" : value}</strong><span>{metric.ratingMax ?? 10}</span></div></div>}{metric.type === "boolean" && <div className="boolean"><button type="button" className={value === true ? "selected" : ""} onClick={() => setValue(key, true)}>Yes</button><button type="button" className={value === false ? "selected" : ""} onClick={() => setValue(key, false)}>No</button></div>}{metric.type === "number" && <label className="number-input"><input aria-label={metric.name} type="number" inputMode="decimal" step="any" value={String(value ?? "")} onChange={(event) => setValue(key, event.target.value)} /><span>{metric.unit}</span></label>}{metric.type === "text" && <textarea aria-label={metric.name} value={String(value ?? "")} onChange={(event) => setValue(key, event.target.value)} placeholder="Write a short note…" />}{metric.type === "choice" && <select aria-label={metric.name} value={String(value ?? "")} onChange={(event) => setValue(key, event.target.value)}><option value="">Choose one…</option>{metric.options?.map((option) => <option key={option}>{option}</option>)}</select>}</div></article>; }))}</section><footer><button type="button" className="secondary" onClick={onClose}>Cancel</button><button className="primary" type="submit">Save entries</button></footer></form></AppDialog>;
}
...
function Insights({ metrics, entries }: { metrics: Metric[]; entries: Entry[] }) {
  ...
  const chartLabel = (value: unknown, metricIds: string[]) => {
    const key = String(value);
    const periodEntries = rangedEntries.filter((entry) => {
      if (!metricIds.includes(entry.metricId) || !entry.note) return false;
      if (granularity === "day") return entry.localDate === key;
      if (granularity === "month") return entry.localDate.startsWith(key);
      const date = new Date(`${entry.localDate}T12:00:00`); date.setDate(date.getDate() - ((date.getDay() + 6) % 7));
      return date.toISOString().slice(0, 10) === key;
    });
    return <div className="chart-tooltip-label"><strong>{chartDate(key)}</strong>{periodEntries.map((entry) => <span key={entry.id}>{metrics.find((metric) => metric.id === entry.metricId)?.name}: {entry.note}</span>)}</div>;
  };
  return <div className="page"><PageHeader eyebrow="Explore gently" title="Insights" copy="Compare the signals in your life. Patterns are clues—not proof." />
    <div className="insight-toolbar"><div><span>Range</span>{(["30", "90", "365", "all"] as const).map((value) => <button aria-pressed={range === value} className={range === value ? "selected" : ""} key={value} onClick={() => setRange(value)}>{value === "all" ? "All" : `${value}d`}</button>)}</div><div><span>Group by</span>{(["day", "week", "month"] as const).map((value) => <button aria-pressed={granularity === value} className={granularity === value ? "selected" : ""} key={value} onClick={() => setGranularity(value)}>{value}</button>)}</div></div>
    ...
    <section className="chart-grid"><article className="chart-card"><header><div><span>Trend</span><h2>{a?.name}</h2></div><span className="range-pill">{range === "all" ? "All data" : `${range} days`}</span></header><div className="chart"><ResponsiveContainer width="100%" height="100%"><AreaChart data={seriesA}><defs><linearGradient id="area" x1="0" y1="0" x2="0" y2="1"><stop offset="0%" stopColor="var(--chart-primary, #47715b)" stopOpacity={.3}/><stop offset="100%" stopColor="var(--chart-primary, #47715b)" stopOpacity={0}/></linearGradient></defs><CartesianGrid vertical={false} stroke="#e5e0d5" /><XAxis dataKey="date" tickFormatter={chartDate} tickLine={false} axisLine={false} fontSize={11} /><YAxis tickLine={false} axisLine={false} fontSize={11} width={25} /><Tooltip labelFormatter={(label) => chartLabel(label, a ? [a.id] : [])} /><Area type="monotone" dataKey="value" stroke="var(--chart-primary, #47715b)" strokeWidth={2.5} fill="url(#area)" /></AreaChart></ResponsiveContainer></div></article>
      <article className="chart-card"><header><div><span>Shared timeline</span><h2>{a?.name} & {b?.name}</h2></div></header><div className="chart"><ResponsiveContainer width="100%" height="100%"><LineChart data={result.points}><CartesianGrid vertical={false} stroke="#e5e0d5" /><XAxis dataKey="date" tickFormatter={chartDate} tickLine={false} axisLine={false} fontSize={11} /><YAxis yAxisId="left" hide domain={["auto", "auto"]} /><YAxis yAxisId="right" hide orientation="right" domain={["auto", "auto"]} /><Tooltip labelFormatter={(label) => chartLabel(label, [a?.id, b?.id].filter((id): id is string => Boolean(id)))} /><Line yAxisId="left" type="monotone" dataKey="x" name={a?.name} stroke="var(--chart-primary, #47715b)" strokeWidth={2.5} dot={false} /><Line yAxisId="right" type="monotone" dataKey="y" name={b?.name} stroke="var(--chart-secondary, #b66b2c)" strokeWidth={2.5} dot={false} /></LineChart></ResponsiveContainer></div></article>
      <article className="chart-card scatter-card"><header><div><span>Relationship</span><h2>{a?.name} vs. {b?.name}</h2></div></header><div className="chart"><ResponsiveContainer width="100%" height="100%"><ScatterChart margin={{ bottom: 8, left: 0 }}><CartesianGrid stroke="#e5e0d5" /><XAxis type="number" dataKey="x" name={a?.name} tickLine={false} fontSize={11} /><YAxis type="number" dataKey="y" name={b?.name} tickLine={false} fontSize={11} /><ZAxis range={[55, 55]} /><Tooltip cursor={{ strokeDasharray: "3 3" }} labelFormatter={(label, payload) => chartLabel(payload[0]?.payload?.date ?? label, [a?.id, b?.id].filter((id): id is string => Boolean(id)))} /><Scatter data={result.points} fill="var(--chart-primary, #47715b)" /></ScatterChart></ResponsiveContainer></div></article>
...
function ReminderSettings({ userId }: { userId: string }) {
  ...
  const [subscription, setSubscription] = useState<PushSubscription | null>(null); const [error, setError] = useState(""); const [installPrompt, setInstallPrompt] = useState<InstallPromptEvent | null>(null); const [dailyEnabled, setDailyEnabled] = useState(false); const [dailyTime, setDailyTime] = useState("20:00");
  useEffect(() => { if (!supported) return; void navigator.serviceWorker.ready.then(async (registration) => { setSubscription(await registration.pushManager.getSubscription()); }); }, [supported, userId]);
  useEffect(() => { void getSupabase()?.from("profiles").select("daily_reminder_enabled, reminder_time").eq("id", userId).single().then(({ data }) => { if (!data) return; setDailyEnabled(Boolean(data.daily_reminder_enabled)); setDailyTime(String(data.reminder_time).slice(0, 5)); }); }, [userId]);
  ...
  const saveDailyReminder = async (enabled: boolean, time = dailyTime) => { setError(""); const { error: saveError } = await getSupabase()!.from("profiles").update({ daily_reminder_enabled: enabled, reminder_time: time }).eq("id", userId); if (saveError) setError(saveError.message); else setDailyEnabled(enabled); };
  return <><section className="settings-card"><header><span><Download size={20} /></span><div><h2>Install LifeLog</h2><p>Add it to your home screen for an app-like experience and iPhone notifications.</p></div></header><div className="settings-actions">{installPrompt ? <button className="primary" onClick={async () => { await installPrompt.prompt(); setInstallPrompt(null); }}>Install app</button> : <span className="settings-hint">On iPhone: tap Share, then “Add to Home Screen.”</span>}</div></section><section className="settings-card"><header><span><CalendarDays size={20} /></span><div><h2>Scheduled reminders</h2><p>Receive metric-specific prompts and an optional daily check-in reminder.</p></div></header>{supported ? <div className="reminder-controls">{subscription && <><label className="notes-toggle"><input type="checkbox" checked={dailyEnabled} onChange={(event) => void saveDailyReminder(event.target.checked)} /><span><strong>Daily check-in reminder</strong><small>Remind me to fill out today’s metrics.</small></span></label><label>Reminder time<input type="time" value={dailyTime} disabled={!dailyEnabled} onChange={(event) => { setDailyTime(event.target.value); void saveDailyReminder(dailyEnabled, event.target.value); }} /></label></>}<div className="settings-actions">{subscription ? <><button className="secondary" onClick={test}>Send test</button><button className="danger" onClick={disable}>Turn off</button></> : <button className="primary" onClick={enable}>Enable reminders</button>}</div>{error && <p className="setting-error">{error}</p>}</div> : <p className="settings-hint">Install LifeLog and use a supported browser to enable reminders.</p>}</section></>;
}
...
export function AddMetricModal({ initial, onClose, onAdd }: { initial?: Metric; onClose: () => void; onAdd: (metric: Metric) => void }) {
  const [name, setName] = useState(initial?.name ?? ""); const [description, setDescription] = useState(initial?.description ?? ""); const [type, setType] = useState<MetricType>(initial?.type ?? "number"); const [mode, setMode] = useState<LoggingMode>(initial?.loggingMode ?? "daily"); const [schedule, setSchedule] = useState<ScheduleType>(initial?.schedule ?? "daily"); const [unit, setUnit] = useState(initial?.unit ?? ""); const [options, setOptions] = useState(initial?.options?.join(", ") ?? ""); const [color, setColor] = useState<MetricColor>(initial?.color ?? "sage"); const [ratingMin, setRatingMin] = useState(String(initial?.ratingMin ?? 1)); const [ratingMax, setRatingMax] = useState(String(initial?.ratingMax ?? 10)); const [aggregation, setAggregation] = useState<Metric["aggregation"]>(initial?.aggregation ?? "sum"); const [weekdays, setWeekdays] = useState(initial?.weekdays ?? [1, 2, 3, 4, 5]); const [notesEnabled, setNotesEnabled] = useState(initial?.notesEnabled ?? false); const [frequency, setFrequency] = useState<FrequencyType>(initial?.frequency === "interval" ? "times" : initial?.frequency ?? "once"); const [timesPerDay, setTimesPerDay] = useState(String(initial?.frequency === "interval" ? metricSlots(initial).length : initial?.timesPerDay ?? 3)); const [scheduleTimes, setScheduleTimes] = useState(initial?.frequency === "interval" ? metricSlots(initial).map((slot) => slot.key) : initial?.scheduleTimes ?? []); const [notificationsEnabled, setNotificationsEnabled] = useState(initial?.notificationsEnabled ?? false);
  const submit = (event: React.FormEvent) => { event.preventDefault(); const choiceOptions = options.split(",").map((value) => value.trim()).filter(Boolean); if (!name.trim() || (type === "choice" && choiceOptions.length < 2) || (type === "rating" && (!ratingMin.trim() || !ratingMax.trim() || Number(ratingMax) <= Number(ratingMin)))) return; onAdd({ id: initial?.id ?? crypto.randomUUID(), name: name.trim(), description: description.trim() || undefined, type, unit: unit.trim() || undefined, loggingMode: mode, schedule, weekdays: schedule === "weekdays" ? weekdays : undefined, color, ratingMin: type === "rating" ? Number(ratingMin) : undefined, ratingMax: type === "rating" ? Number(ratingMax) : undefined, options: type === "choice" ? choiceOptions : undefined, aggregation: mode === "event" ? aggregation : undefined, notesEnabled, frequency: mode === "daily" ? frequency : undefined, timesPerDay: frequency === "times" ? Number(timesPerDay) : undefined, scheduleTimes: mode === "daily" ? scheduleTimes.slice(0, timeCount).filter(Boolean) : undefined, notificationsEnabled: mode === "daily" && notificationsEnabled && scheduleTimes.some(Boolean), archived: initial?.archived }); };
  const pattern: "once" | "times" | "event" = mode === "event" ? "event" : frequency === "times" ? "times" : "once";
  const setPattern = (value: "once" | "times" | "event") => { if (value === "event") { setMode("event"); setSchedule("flexible"); setNotificationsEnabled(false); } else { if (mode === "event") setSchedule("daily"); setMode("daily"); setFrequency(value); } setScheduleTimes([]); };
  const timeCount = frequency === "times" ? Math.max(0, Math.min(12, Math.trunc(Number(timesPerDay) || 0))) : 1;
  const setScheduleTime = (index: number, value: string) => setScheduleTimes((current) => Array.from({ length: timeCount }, (_, item) => item === index ? value : current[item] ?? ""));
  return <AppDialog onClose={onClose}><form className="modal metric-form" onSubmit={submit}><header><div><span className="eyebrow">{initial ? "Metric settings" : "New question"}</span><h2 tabIndex={-1}>{initial ? `Edit ${initial.name}` : "What would you like to notice?"}</h2></div><button type="button" aria-label="Close" onClick={onClose}><X size={20} /></button></header><label>Name<input required value={name} onChange={(event) => setName(event.target.value)} placeholder="e.g. Hours of sleep" /></label><label>Description <span>(optional)</span><input value={description} onChange={(event) => setDescription(event.target.value)} placeholder="A short prompt or reminder" /></label><div className="form-row"><label>Answer type<select disabled={Boolean(initial)} value={type} onChange={(event) => { const nextType = event.target.value as MetricType; setType(nextType); if (nextType !== "number") setMode("daily"); }}><option value="number">Number</option><option value="rating">Rating scale</option><option value="boolean">Yes / no</option><option value="choice">Single choice</option><option value="text">Text</option></select></label><label>Logging pattern<select value={pattern} onChange={(event) => setPattern(event.target.value as "once" | "times" | "event")}><option value="once">One check-in per day</option><option value="times">Several check-ins per day</option>{type === "number" && <option value="event">Log whenever it happens</option>}</select></label></div>{pattern === "event" && <p className="form-help">Use this for things like cups of coffee or glasses of water. Add a log each time it happens; LifeLog combines those logs into one daily chart value.</p>}{type === "number" && <label>Unit <span>(optional)</span><input value={unit} onChange={(event) => setUnit(event.target.value)} placeholder="cups, hours, miles…" /></label>}{type === "rating" && <div className="form-row"><label>Minimum<input required type="number" inputMode="decimal" value={ratingMin} onChange={(event) => setRatingMin(event.target.value)} /></label><label>Maximum<input required type="number" inputMode="decimal" value={ratingMax} onChange={(event) => setRatingMax(event.target.value)} /></label></div>}{type === "choice" && <label>Choices <span>(at least two, comma separated)</span><input required value={options} onChange={(event) => setOptions(event.target.value)} placeholder="Great, okay, difficult" /></label>}{mode === "event" && <label>Daily chart value <span>(for logs made throughout the day)</span><select value={aggregation} onChange={(event) => setAggregation(event.target.value as Metric["aggregation"])}><option value="sum">Add the logged values</option><option value="count">Count the number of logs</option><option value="average">Average the logged values</option></select></label>}<label className="notes-toggle notes-setting"><input type="checkbox" checked={notesEnabled} onChange={(event) => setNotesEnabled(event.target.checked)} /><span><strong>Allow notes</strong><small>Add optional context to each entry.</small></span></label>{mode === "daily" && <section className="schedule-builder">{frequency === "times" && <label>Number of check-ins (2–12 per day)<input required type="number" inputMode="decimal" min="2" max="12" step="1" value={timesPerDay} onChange={(event) => { setTimesPerDay(event.target.value); }} /></label>}<div className="schedule-times"><strong>Check-in times (optional)</strong>{Array.from({ length: timeCount }, (_, index) => <label key={index}>{frequency === "times" ? `Check-in ${index + 1}` : "Time"}<input type="time" value={scheduleTimes[index] ?? ""} onChange={(event) => setScheduleTime(index, event.target.value)} /></label>)}</div><label className="notes-toggle"><input type="checkbox" checked={notificationsEnabled} disabled={!scheduleTimes.some(Boolean)} onChange={(event) => setNotificationsEnabled(event.target.checked)} /><span><strong>Push reminders</strong><small>{scheduleTimes.some(Boolean) ? "Notify at these times when notifications are enabled." : "Add a time to enable reminders."}</small></span></label></section>}{mode === "daily" && <label>Days<select value={schedule} onChange={(event) => setSchedule(event.target.value as ScheduleType)}><option value="daily">Every day</option><option value="weekdays">Selected days</option><option value="flexible">Only when I choose</option></select></label>}{mode === "daily" && schedule === "weekdays" && <div className="weekday-picker">{["S", "M", "T", "W", "T", "F", "S"].map((label, index) => <button type="button" key={`${label}-${index}`} aria-label={["Sunday", "Monday", "Tuesday", "Wednesday", "Thursday", "Friday", "Saturday"][index]} aria-pressed={weekdays.includes(index)} className={weekdays.includes(index) ? "selected" : ""} onClick={() => setWeekdays((current) => current.includes(index) ? current.filter((day) => day !== index) : [...current, index])}>{label}</button>)}</div>}<fieldset className="color-picker"><legend>Color</legend>{(Object.keys(COLORS) as MetricColor[]).map((option) => <button type="button" aria-pressed={color === option} aria-label={option} key={option} className={color === option ? "selected" : ""} style={{ background: COLORS[option] }} onClick={() => setColor(option)} />)}</fieldset><footer><button type="button" className="secondary" onClick={onClose}>Cancel</button><button className="primary" type="submit">{initial ? "Save changes" : "Create metric"}</button></footer></form></AppDialog>;
}
