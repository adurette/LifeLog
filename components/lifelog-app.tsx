...
function QuickLog({ metric, entries, onSave }: { metric: Metric; entries: Entry[]; onSave: (metric: Metric, value: EntryValue, date?: string, note?: string) => void }) { const [amount, setAmount] = useState("1"); const [note, setNote] = useState(""); const total = entries.reduce((sum, entry) => sum + Number(entry.value), 0); const save = () => { if (!amount.trim() || !Number.isFinite(Number(amount))) return; onSave(metric, Number(amount), undefined, note); setNote(""); }; return <article className={`quick-card ${metric.notesEnabled ? "with-note" : ""}`} style={{ "--metric": COLORS[metric.color] } as React.CSSProperties}><span className="quick-icon"><MetricGlyph metric={metric} size={21} /></span><div><strong>{metric.name}</strong><span>{total} {metric.unit} today</span></div><input aria-label={`${metric.name} amount`} type="number" step="any" value={amount} onChange={(event) => setAmount(event.target.value)} /><button aria-label={`Log ${metric.name}`} disabled={!amount.trim() || !Number.isFinite(Number(amount))} onClick={save}><Plus size={20} /></button>{metric.notesEnabled && <input className="quick-note" aria-label={`${metric.name} notes`} maxLength={500} value={note} onChange={(event) => setNote(event.target.value)} placeholder="Optional note…" />}</article>; }
...
function EditEntryModal({ entry, metric, onClose, onSave }: { entry: Entry; metric: Metric; onClose: () => void; onSave: (value: EntryValue, note?: string) => void }) {
  ...
  const [note, setNote] = useState(entry.note ?? "");
  return <div className="modal-backdrop" onMouseDown={(event) => { if (event.currentTarget === event.target) onClose(); }}><form className="modal entry-modal" onSubmit={(event) => { event.preventDefault(); if (validEntryValue(metric, value)) onSave(metric.type === "number" ? Number(value) : value, note); }}><header><div><span className="eyebrow">{prettyDate(entry.localDate)}</span><h2>Edit {metric.name}</h2></div><button type="button" aria-label="Close" onClick={onClose}><X size={20} /></button></header><div className="metric-control">{metric.type === "rating" && <div className="rating"><input aria-label={metric.name} type="range" min={metric.ratingMin ?? 1} max={metric.ratingMax ?? 10} value={Number(value)} onChange={(event) => setValue(Number(event.target.value))} /><div><span>{metric.ratingMin ?? 1}</span><strong>{value}</strong><span>{metric.ratingMax ?? 10}</span></div></div>}{metric.type === "boolean" && <div className="boolean"><button type="button" className={value === true ? "selected" : ""} onClick={() => setValue(true)}>Yes</button><button type="button" className={value === false ? "selected" : ""} onClick={() => setValue(false)}>No</button></div>}{metric.type === "number" && <label className="number-input"><input aria-label={metric.name} type="number" required step="any" value={String(value)} onChange={(event) => setValue(event.target.value)} /><span>{metric.unit}</span></label>}{metric.type === "text" && <textarea aria-label={metric.name} value={String(value)} onChange={(event) => setValue(event.target.value)} />}{metric.type === "choice" && <select aria-label={metric.name} value={String(value)} onChange={(event) => setValue(event.target.value)}>{metric.options?.map((option) => <option key={option}>{option}</option>)}</select>}</div>{metric.notesEnabled && <NoteField value={note} onChange={setNote} />}<footer><button type="button" className="secondary" onClick={onClose}>Cancel</button><button className="primary" type="submit">Save changes</button></footer></form></div>;
}
...
function BackfillModal({ metrics, onClose, onSave }: { metrics: Metric[]; onClose: () => void; onSave: (metric: Metric, value: EntryValue, date: string, note?: string, slotKey?: string) => void }) {
  ...
  return <div className="modal-backdrop" onMouseDown={(event) => { if (event.currentTarget === event.target) onClose(); }}><form className="modal entry-modal" onSubmit={(event) => { event.preventDefault(); if (validEntryValue(metric, value)) onSave(metric, metric.type === "number" ? Number(value) : value, date, note, slotKey); }}><header><div><span className="eyebrow">Backfill your record</span><h2>Add a past entry</h2></div><button type="button" aria-label="Close" onClick={onClose}><X size={20} /></button></header><div className="form-row"><label>Metric<select value={metricId} onChange={(event) => chooseMetric(event.target.value)}>{metrics.map((item) => <option value={item.id} key={item.id}>{item.name}</option>)}</select></label><label>Date<input required type="date" max={day()} value={date} onChange={(event) => setDate(event.target.value)} /></label></div>{metricSlots(metric).length > 1 && <label>Scheduled check-in<select value={slotKey} onChange={(event) => setSlotKey(event.target.value)}>{metricSlots(metric).map((slot) => <option key={slot.key} value={slot.key}>{slot.label}</option>)}</select></label>}<div className="metric-control">{metric.type === "rating" && <div className="rating"><input aria-label={metric.name} type="range" min={metric.ratingMin ?? 1} max={metric.ratingMax ?? 10} value={Number(value)} onChange={(event) => setValue(Number(event.target.value))} /><div><span>{metric.ratingMin ?? 1}</span><strong>{value}</strong><span>{metric.ratingMax ?? 10}</span></div></div>}{metric.type === "boolean" && <div className="boolean"><button type="button" className={value === true ? "selected" : ""} onClick={() => setValue(true)}>Yes</button><button type="button" className={value === false ? "selected" : ""} onClick={() => setValue(false)}>No</button></div>}{metric.type === "number" && <label className="number-input"><input aria-label={metric.name} type="number" required step="any" value={String(value)} onChange={(event) => setValue(event.target.value)} /><span>{metric.unit}</span></label>}{metric.type === "text" && <textarea required aria-label={metric.name} value={String(value)} onChange={(event) => setValue(event.target.value)} placeholder="Write a short note…" />}{metric.type === "choice" && <select aria-label={metric.name} value={String(value)} onChange={(event) => setValue(event.target.value)}><option value="">Choose one…</option>{metric.options?.map((option) => <option key={option}>{option}</option>)}</select>}</div>{metric.notesEnabled && <NoteField value={note} onChange={setNote} />}<footer><button type="button" className="secondary" onClick={onClose}>Cancel</button><button className="primary" type="submit">Save entry</button></footer></form></div>;
}
