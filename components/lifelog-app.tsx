...
function useLifeLog(userId: string | null) {
  ...
  const updateEntry = (entry: Entry, value: EntryValue, note?: string) => { const metric = data.metrics.find((item) => item.id === entry.metricId); if (!metric || !validEntryValue(metric, value)) return; const cleanNote = metric.notesEnabled ? note?.trim() || undefined : entry.note; const updated = { ...entry, value, note: cleanNote, synced: false }; setData((current) => ({ ...current, entries: current.entries.map((item) => item.id === entry.id ? updated : item) })); send({ id: `entry:${entry.id}`, kind: "entry", payload: { id: entry.id, user_id: userId, metric_id: entry.metricId, value, occurred_at: entry.occurredAt, local_date: entry.localDate, timezone: entry.timezone, slot_key: metric.loggingMode === "daily" ? entry.slotKey ?? "daily" : entry.id, note: cleanNote } }); };
...
function metricSlots(metric: Metric) {
  if (metric.loggingMode === "event") return [];
...
function BackfillModal({ metrics, onClose, onSave }: { metrics: Metric[]; onClose: () => void; onSave: (metric: Metric, value: EntryValue, date: string, note?: string, slotKey?: string) => void }) {
  ...
  return <AppDialog onClose={onClose}><form className="modal entry-modal" onSubmit={(event) => { event.preventDefault(); if (validEntryValue(metric, value)) onSave(metric, metric.type === "number" ? Number(value) : value, date, note, slotKey); }}><header><div><span className="eyebrow">Backfill your record</span><h2 tabIndex={-1}>Add a past entry</h2></div><button type="button" aria-label="Close" onClick={onClose}><X size={20} /></button></header><div className="form-row"><label>Metric<select value={metricId} onChange={(event) => chooseMetric(event.target.value)}>{metrics.map((item) => <option value={item.id} key={item.id}>{item.name}</option>)}</select></label><label>Date<input required type="date" max={day()} value={date} onChange={(event) => setDate(event.target.value)} /></label></div>{metricSlots(metric).length > 1 && <label>Scheduled check-in<select value={slotKey} onChange={(event) => setSlotKey(event.target.value)}>{metricSlots(metric).map((slot) => <option key={slot.key} value={slot.key}>{slot.label}</option>)}</select></label>}<div className="metric-control">{metric.type === "rating" && <div className="rating" style={{ "--range-fill": `${((Number(value) - (metric.ratingMin ?? 1)) / Math.max(1, (metric.ratingMax ?? 10) - (metric.ratingMin ?? 1))) * 100}%` } as React.CSSProperties}><input aria-label={metric.name} type="range" min={metric.ratingMin ?? 1} max={metric.ratingMax ?? 10} value={Number(value)} onChange={(event) => setValue(Number(event.target.value))} /><div><span>{metric.ratingMin ?? 1}</span><strong>{value}</strong><span>{metric.ratingMax ?? 10}</span></div></div>}{metric.type === "boolean" && <div className="boolean"><button type="button" className={value === true ? "selected" : ""} onClick={() => setValue(true)}>Yes</button><button type="button" className={value === false ? "selected" : ""} onClick={() => setValue(false)}>No</button></div>}{metric.type === "number" && <label className="number-input"><input aria-label={metric.name} type="number" inputMode="decimal" required step="any" value={String(value)} onChange={(event) => setValue(event.target.value)} /><span>{metric.unit}</span></label>}{metric.type === "text" && <textarea required aria-label={metric.name} value={String(value)} onChange={(event) => setValue(event.target.value)} placeholder="Write a short note…" />}{metric.type === "choice" && <select aria-label={metric.name} value={String(value)} onChange={(event) => setValue(event.target.value)}><option value="">Choose one…</option>{metric.options?.map((option) => <option key={option}>{option}</option>)}</select>}</div>{metric.notesEnabled && <NoteField value={note} onChange={setNote} />}<footer><button type="button" className="secondary" onClick={onClose}>Cancel</button><button className="primary" type="submit">Save entry</button></footer></form></AppDialog>;
}
